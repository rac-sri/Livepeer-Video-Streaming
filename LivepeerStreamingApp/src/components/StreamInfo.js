...
class StreamInfo extends Component {
  ...
  componentDidMount() {
    ...
    LivepeerSDK({
      ...
      controllerAddress: config.get("controllerAddress"),
    }).then(async (sdk) => {
      const { rpc } = sdk;
      const jobs = await rpc.getJobs({ broadcaster: config.get("ETHAddress") });
      const job = jobs.filter(
        (job) => job.streamId.substring(0, 132) == this.props.streamId
      );
  ...
  totalBroadcastPrice() {
    if (
      this.state.pricePerSegment == "can't find job" ||
      this.state.pricePerSegment == "unknown"
    ) {
